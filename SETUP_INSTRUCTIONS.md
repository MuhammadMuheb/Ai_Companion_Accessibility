...
Double-click **`nova.pyw`** — no terminal, no browser, no server, and **no window**: Nova starts invisibly
and waits for its name ("Hello MD", "Hey Nova" — whatever you named it). Answers come back by voice.
Its window is only built when you ask for it ("open your window", Ctrl+Alt+N, or the tray menu).
`nova.pyw --show` (the Start-menu shortcut) opens the window straight away. `python main.py` = invisible start too.

- Nova lives in the **system tray** (near the clock). Its menu: *Open*, *Talk now*, *Settings*,
  *Pause listening*, *Start with Windows*, *Show in Start menu*, *Quit*.
- **Ctrl+Alt+N** opens Nova's window from anywhere; launching `nova.pyw` again while Nova runs
  just brings the window to the front (only one Nova ever runs).
- Closing the window keeps Nova running in the tray; *Quit* in the tray menu stops it.
- Tick **Start with Windows** (tray or Settings → Background) and Nova starts quietly in the tray
  every time you sign in. **Show in Start menu** lets you find it by searching “Nova”.
- The window is a native WebView2 window (the Windows component WhatsApp Desktop uses), so screen
  ...
  Nova's functions directly inside the same process.

The window has these sections:

- **Chat** — press 🎤 (or `Ctrl+Space`), wait for the beep, speak — Nova uses the same microphone as
...
## Talking to Nova (a conversation, not one command)

Say the name once — "Hello MD" — and Nova answers "Ji?". After every answer it beeps and listens
again, so you can keep talking without repeating its name:

1. You: "Hello MD" → Nova: "Ji?"  2. "what time is it" → answer, beep  3. "open notepad" → done, beep
...
- Noise or gibberish (coughs, "the the the", Whisper's invented "Thank you") is skipped; twice in a
  row and Nova says "Samajh nahi aaya" and stops.
...
| What | Say |
|---|---|
| Rename Nova | "your name is Nova", "call yourself Zoya" (asks first; the wake phrase changes to the new name) |
...
Long answers and code are not read out character by character: Nova says a short version and puts the
...
Nova keeps facts, preferences, goals, projects and people in `data/memory.db` (local SQLite on this
...
Nova stays silent during calls (nothing is spoken into the call). The wake word still works for call
...
## Nova: background, calls, voice print, pointer, system control, expert mode

**Name.** The assistant is called *Nova*; change it under Settings → About you and the wake word
follows (“Hey Zoya”). Custom wake names can still be listed under Settings → Voice.

**Microphone switching.** Nova records through Windows' own audio system and follows the Windows
...
on only enrolled voices can wake Nova (strangers get “main sirf pehchaani hui awaaz par kaam karti
hoon”). “Hey Nova” alone is too short to recognise a voice, so Nova checks the name *plus* the
...
starts using the microphone, Nova goes silent (no speech, no wake word) and a small pop-up asks
...
ends Nova saves `data/calls/<date>/me.wav, them.wav, transcript.txt, notes.md`, tells you the summary
and action items (offers to add them as goals), asks how the call went and suggests water / a break.

**Running in the background.** Double-click `nova.pyw` (tray icon with Open /
Pause listening / Start with Windows / Quit), or tick *Start Nova … when I sign in* in Settings. While
Windows is **locked** Nova keeps listening and can answer questions, set reminders, read
...
reading” — rest the pointer anywhere and Nova names it. “Show me the Save button” — glides the mouse
...
packages, disk and Nova's own log; offers safe fixes (start Ollama, download a model, pip-install a
package) and explains logged errors with the source line and the exact commands to run. Nova does
...
set a weekly US$ limit — Nova counts every request's tokens and falls back to the local model when
...
means exposing Nova (which can control this computer) on your network and serving HTTPS for the
