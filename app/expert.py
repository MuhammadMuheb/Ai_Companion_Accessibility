...
- "claude": Claude via the Anthropic API, if you have set up credentials yourself
  (ANTHROPIC_API_KEY or `ant auth login`). Nova never asks for or stores the key.

A weekly spending cap (Settings → Expert) is enforced from the token counts the API reports;
once it is reached Nova falls back to the local model until the next week starts.
"""
...
EXPERT_SYSTEM = """You are Nova's expert mode: a principal software engineer, web architect and product designer.
