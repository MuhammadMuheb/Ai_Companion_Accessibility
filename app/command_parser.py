...
RULES: list[tuple[re.Pattern, str, callable]] = [(re.compile(p, re.I), name, fn) for p, name, fn in [
    ...
    # ---- self-diagnosis ----
    (r"^(?:diagnose|check|test|heal|fix) (?:yourself|your ?self|nova|the companion|the system)|^self[- ]?(?:check|test|heal|diagnos\w*)|"
    ...
    (r"^(?:open|show)(?: me)? (?:your |nova'?s |the |my )?(window|settings|chat|memory|commands|accounts)(?: window| page)?$|"
