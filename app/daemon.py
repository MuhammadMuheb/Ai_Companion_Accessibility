"""Nova as a headless Windows background app — no web server, no browser, no terminal,
and no window unless you ask for one.

One process holds everything: the companion, scheduler, voice (wake word, speech), the
background services (notifications, calls, hotkeys, watchdog) and a system-tray icon. Nova
starts invisible and is used by voice ("Hello Nova ..."). Its window (chat, settings, commands,
accounts, memory) is only created the first time you open it from the tray, with Ctrl+Alt+N, or
by launching Nova again — until then not even the WebView is loaded.
...
listens on the network: the page calls Nova's functions directly inside this process (the API
routes are invoked in-process, without any socket or port).

Start: `nova.pyw` (double-click, no console), the Start-menu / startup shortcut, or `python main.py`.
Launching Nova again while it runs just brings its window to the front.
"""
...
def page_html() -> str:
    ...
    html = html.replace('<script src="/static/app.js"></script>',
                        f"<script>window.NOVA_NATIVE = true;</script>\n<script>\n{js}\n</script>")
    return html
...
class NovaApp:
    ...
    def show(self, tab: str | None = None) -> None:
        """Open Nova's window — from the tray, Ctrl+Alt+N, a second launch, or a voice command."""
    ...
    def _on_closing(self):
        if self._quitting:
            return True
        self.window.hide()   # closing the window keeps Nova running invisibly
        return False
    ...
    def run_ui_when_needed(self) -> None:
        ...
        log.info("Opening Nova's window")
        webview.start(gui="edgechromium", private_mode=False, storage_path=str(storage))

    # ---- lifecycle -------------------------------------------------------------------------------
    def start_background(self) -> None:
        threading.Thread(target=self.state.start_background, daemon=True, name="nova-start").start()
    ...
    def quit(self) -> None:
        ...
        try:
            ...
            log.info("Nova stopped from the tray")
            os._exit(0)
...
def run(show_window: bool = False) -> int:
    ...
    with SingleInstance() as instance:
        if not instance.primary:
            ...
            log.info("Nova already running; asked it to show its window")
            return 0
        make_dpi_aware()
        _enable_crash_log()
        app = NovaApp()
        app.start_background()
        instance.on_show_request(app.show)   # launching Nova again = "show me the window"
        start_tray(app)
        log.info("Nova running invisibly in the background (no window, no server)")
        if show_window:
            app.show()
