...
SETTINGS_FILE = ROOT / "data" / "settings.yaml"
...
class AssistantConfig:
    name: str = "Nova"  # what the assistant calls itself; also the default wake word
...
class HotkeysConfig:
    ...
    open_window: str = "<ctrl>+<alt>+n"  # open Nova's chat window
...
class Config:
    ...
    def wake_phrases(self) -> list[str]:
        ...
        if phrases:
            return phrases
        name = (self.assistant.name or "Nova").strip()
...
def load_config(path: str | Path | None = None, use_settings: bool = True) -> Config:
    cfg = Config()
    _apply_file(cfg, Path(path) if path else ROOT / "config.yaml")
    if load_dotenv:
