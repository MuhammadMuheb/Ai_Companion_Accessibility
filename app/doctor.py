...
notification database) and `recent_errors()` pulls real tracebacks out of Nova's own log and
points at the source line. Problems with a known, safe fix (start Ollama, pull a missing model,
pip-install a missing package) are fixed after the user says yes. For anything else Nova explains
...
PACKAGES = {  # import name -> pip name, feature
    ...
    "fastapi": ("fastapi", "Nova's window (in-process API)"),
    "webview": ("pywebview", "Nova's native window"),
}
