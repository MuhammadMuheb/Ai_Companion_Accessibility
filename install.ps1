# Nova installer for Windows - right-click > "Run with PowerShell" (no administrator rights needed).
# Installs the Python packages and the local AI models, then adds Nova to the Start menu and to
...
Step "Adding Nova to the Start menu and to Windows sign-in"
...
Write-Host "`nDone! Starting Nova in the background - say 'Hello Nova'." -ForegroundColor Green
$pythonw = (& $exe @pyArgs -c "import sys, pathlib; print(pathlib.Path(sys.executable).with_name('pythonw.exe'))").Trim()
Start-Process -FilePath $pythonw -ArgumentList "`"$PSScriptRoot\nova.pyw`"" -WorkingDirectory $PSScriptRoot
