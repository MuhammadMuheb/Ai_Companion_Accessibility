"""Tests for Nova's newer parts: naming, microphone switching, voice print gate, calls,
...
def test_wake_phrases_follow_assistant_name(cfg, monkeypatch):
    ...
    monkeypatch.setattr(cfg.assistant, "name", "Nova")
    assert cfg.wake_phrases == ["hey nova", "nova"]
...
def test_wake_gate_accepts_owner_and_combines_short_wake(vp, monkeypatch):
    ...
    services = make_services(voice)
    services.handle_wake("", clip(0)[:8000])          # "Hey Nova" alone: 0.5 s
