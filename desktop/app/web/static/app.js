...
// In the native MD window there is no server: calls go through the in-process bridge.
const NATIVE = !!window.MD_NATIVE;
...
async function listenOnce(statusEl = $("#mic-status")) {
  if (NATIVE) {  // MD's own microphone (the one the wake word uses); it beeps when to speak
...
addMsg("ai", "Assalam-o-Alaikum! Say “Hey MD”, press the 🎤 button (or Ctrl+Space), or type below.", "MD");
