# Builds MD-Setup-<version>.exe:  powershell -ExecutionPolicy Bypass -File desktop\packaging\build.ps1
# Needs: Python with requirements-FULL.txt + pyinstaller, Inno Setup 6, and the speech models in
# packaging\models (copied from the Hugging Face cache on first build).
...
foreach ($size in @("tiny", "base")) {
    ...
    if (-not (Test-Path "$dest\model.bin")) {
        ...
        if (-not $snap) { throw "Whisper '$size' not in the cache - run MD from source once so it downloads." }
...
python -m PyInstaller "$pkg\md.spec" --noconfirm --clean --distpath "$pkg\dist" --workpath "$pkg\build"
